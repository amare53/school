...
export const generateStudentNumber = (schoolCode: string, year: number): string => {
  const yearStr = year.toString().slice(-2);
  const random = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
  return `${schoolCode}${yearStr}${random}`;
};
...
export const hasPermission = (userRole: string, permission: string, permissions: Record<string, string[]>): boolean => {
...
export const apiPlatformUtils = {
  ...
  normalizeApiPlatformData: <T>(data: any): T => {
    ...
    const { '@context': context, '@id': id, '@type': type, ...cleanData } = data;
    ...
    Object.keys(cleanData).forEach(key => {
      ...
      if (typeof value === 'string' && value.startsWith('/api/')) {
        ...
        cleanData[key] = value.map(item => 
          typeof item === 'object' ? apiPlatformUtils.normalizeApiPlatformData(item) : item
        );
      } else if (typeof value === 'object' && value !== null) {
  ...
  buildApiPlatformFilters: (filters: Record<string, any>): Record<string, any> => {
  ...
  buildApiPlatformSort: (sortBy: string, direction: 'asc' | 'desc'): Record<string, any> => {
    return {
      order: {
        [sortBy]: direction
      }
...
export const getCongoSections = () => {
  return [
    { code: 'MAT', name: 'Maternel', description: 'Petite, moyenne et grande section (3-6 ans)' },
    { code: 'PRI', name: 'Primaire', description: 'CP, CE1, CE2, CM1, CM2 (6-11 ans)' },
    { code: 'SEC', name: 'Secondaire', description: '6ème à Terminale (12-18 ans)' },
  ];
};